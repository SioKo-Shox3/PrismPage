...
- AI エンジンの既存フォルダ登録 / 候補検索 / ZIP 取込 / 状態確認 / 登録解除
...
PrismPage は AI エンジンをアプリに固定同梱せず、**PC 上にある既存のエンジンフォルダを参照登録**して使う構成です。アプリ内にモデルをダウンロードして app local に抱え込むのではなく、RAIV 由来のエンジンや、ユーザーが別途配置したエンジンを登録する方針です。
...
- RAIV ルートや `tools` 配下などから登録候補を検索する
- ZIP アーカイブをアプリへ取り込む
- エンジンの状態確認と登録解除

RAIV を使っている場合は、RAIV のルートフォルダ、または `tools` 配下のエンジンを含むフォルダを選択できます。個人環境の絶対パスを README に固定せず、実際の配置に合わせて選んでください。

ZIP 取込は互換機能として残していますが、推奨は PC 上にある既存フォルダを登録する使い方です。
